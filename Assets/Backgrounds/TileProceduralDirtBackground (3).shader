...
Shader "Custom/ProceduralDirtBackground"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // World-space position instead of UV: this is what makes
                // the pattern scale-independent. A UV of (0,1) always maps
                // to "the whole mesh" no matter its size, so a huge quad
                // would stretch the noise pattern huge too. World position
                // is absolute -- one giant quad and many small tiled quads
                // covering the same physical area sample the exact same
                // noise values at the exact same points, so they look
                // identical and line up seamlessly at tile edges.
                float2 worldUV = IN.positionWS.xy * _NoiseScale;
                ...
                float2 speckleWorldUV = IN.positionWS.xy * _SpeckleScale;
                float2 speckleCell = floor(speckleWorldUV);
